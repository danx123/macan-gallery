...
APP_VERSION = "1.5.0" # [PERUBAHAN] Versi diperbarui dengan fitur navigasi gambar
...
class MacanGallery(QMainWindow):
    def __init__(self):
        ...
        # [FITUR BARU] Variabel untuk navigasi gambar
    ...
    def init_ui(self):
        ...
        # [FITUR BARU] Container untuk overlay navigasi
        self.image_container = QWidget()
        # === PERBAIKAN DIMULAI ===
        # Alih-alih hanya mengatur parent, kita gunakan layout manager
        # agar QScrollArea diatur ukurannya dengan benar di dalam container.
        image_container_layout = QHBoxLayout(self.image_container)
        image_container_layout.setContentsMargins(0, 0, 0, 0)
        image_container_layout.addWidget(self.viewer_scroll_area)
        # === PERBAIKAN SELESAI ===
        
        # [FITUR BARU] Tombol navigasi
        ...
        self.prev_button = QPushButton(self.image_container)
        self.prev_button.setIcon(self._create_svg_icon(prev_svg))
        ...
        self.next_button = QPushButton(self.image_container)
        self.next_button.setIcon(self._create_svg_icon(next_svg))
    ...
    def apply_stylesheet(self):
        self.setStyleSheet("""
            ...
            QPushButton:hover { background-color: #81A1C1; }
            /* [FITUR BARU] Style untuk tombol navigasi */
    ...
    def create_actions(self):
        ...
        # [FITUR BARU] Aksi untuk Navigasi Gambar
        ...
        self.prev_image_action.triggered.connect(self.show_previous_image)
        
        self.addActions([self.zoom_in_action, self.zoom_out_action, self.next_image_action, self.prev_image_action])
    ...
    # [FITUR BARU] Method baru untuk mengurutkan gambar
    def _get_sorted_image_list(self):
        """Returns a sorted list of images in the selected folder based on the current sort method."""
    ...
    def show_image_view(self, path):
        # [FITUR BARU] Tetapkan status navigasi saat ini
        self.current_image_list = self._get_sorted_image_list()
        if path in self.current_image_list:
            self.current_image_index = self.current_image_list.index(path)
            self.current_image_path = path
        else:
            # Jika gambar tidak ditemukan (misalnya dari pencarian), nonaktifkan navigasi
            self.current_image_list = [path]
        ...
        try:
            ...
            # [FITUR BARU] Tampilkan tombol navigasi
            self.prev_button.setVisible(True)
            self.next_button.setVisible(True)
            
            self.zoom_slider.parent().show()
    ...
    def show_gallery_view(self):
        ...
        # [FITUR BARU] Sembunyikan tombol navigasi dan reset state
        self.prev_button.setVisible(False)
        self.next_button.setVisible(False)
        self.current_image_path = None
        self.current_image_list = []
        ...
        self.image_file_type_label.hide()
        self.image_size_label.hide()
        self.zoom_slider.parent().hide()
        self.zoom_label.hide()
        self.zoom_slider.hide()

        self.reflow_ui()

    # [FITUR BARU] Method baru untuk navigasi
    def show_previous_image(self):
        """Navigates to the previous image in the list."""
        if not self.current_image_list or len(self.current_image_list) <= 1:
            return
        
        new_index = (self.current_image_index - 1) % len(self.current_image_list)
        new_path = self.current_image_list[new_index]
        self.show_image_view(new_path)
    ...
    # [FITUR BARU] Method baru untuk memposisikan tombol
    def _update_nav_buttons_position(self):
        """Positions the navigation buttons on the image container."""
        container_size = self.image_container.size()
        button_size = self.prev_button.size()

        y_pos = (container_size.height() - button_size.height()) // 2
        self.prev_button.move(15, y_pos)

        x_pos = container_size.width() - button_size.width() - 15
        self.next_button.move(x_pos, y_pos)
    ...
    def resizeEvent(self, event):
        ...
        if self.main_stack.currentIndex() == 1:
             # [FITUR BARU] Posisikan ulang tombol navigasi saat resize
             # === PERBAIKAN DIMULAI ===
             # Baris ini tidak lagi diperlukan karena layout manager
             # akan menangani resize secara otomatis.
             # self.viewer_scroll_area.resize(self.image_container.size())
             # === PERBAIKAN SELESAI ===
