...
class MacanGallery(QMainWindow):
    def __init__(self):
        ...
        self.original_pixmap_for_editing = None # Menyimpan pixmap asli sebelum rotasi/flip
        self.original_cv_image = None # Menyimpan gambar cv2 asli sebelum editan slider
        ...
        # Dictionary untuk menyimpan nilai slider edit sementara
    ...
    def init_ui(self):
        ...
        # NOTE: Viewer widget tidak lagi menggunakan layout, karena QMainWindow akan mengatur dock
        # dan central widgetnya (image_container) secara otomatis.
        ...
        # --- [PERBAIKAN TOTAL] ---
        # HANYA PANGGIL FUNGSI INI. Jangan buat QDockWidget lagi setelah ini.
        # Fungsi ini akan membuat dock, mengisi widget, dan menambahkannya ke main window.
    ...
    def on_thumbnailing_finished(self):
        ...
        # self.reflow_ui() # Tidak perlu reflow lagi, karena thumbnail sudah diupdate satu per satu
    ...
    def show_image_view(self, path, reload_from_disk=True):
        ...
        try:
            ...
            # [FIX] Menghapus .rgbSwapped() yang tidak perlu karena format sudah BGR888
            ...
            self.viewer_label.setPixmap(QPixmap()) # Clear first
    ...
    def delete_current_image(self):
        ...
        self.show_next_image() # Pindah ke gambar lain dulu
        self.delete_single_image(path_to_delete)
        # Refresh daftar gambar saat ini setelah penghapusan
        self.current_image_list = self._get_filtered_and_sorted_list()
    ...
    def rotate_image(self, angle):
        ...
        # Rotasi pixmap yang disimpan, bukan yang ditampilkan
    ...
    def _save_image_changes_overwrite(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                # Simpan dari pixmap yang sudah dirotasi/flip
                ...
                # Reload cv_image agar sesuai dengan perubahan
                ...
                self.show_image_view(self.current_image_path, reload_from_disk=True) # Revert
        else:
            # Revert
    ...
    def _create_info_dock_widget(self):
        ...
        # Buat widget dan layout untuk menampung tag, lalu masukkan ke scroll area
        ...
        tag_scroll_area.setFixedHeight(100) # Batasi tinggi area tag
        ...
        self.add_tag_button.setFixedSize(30,30)
        ...
        # self.metadata_viewer.setMinimumHeight(150) # Tinggi diatur oleh stretch
    ...
    def apply_image_edits(self, cv_image, edits):
        ...
        # Ambil nilai parameter dari dictionary
        brightness = edits.get('brightness', 0)     # Rentang -100 hingga 100
        contrast = edits.get('contrast', 1.0)       # Rentang 0.0 hingga 2.0
        saturation = edits.get('saturation', 1.0)   # Rentang 0.0 hingga 2.0

        # Jika tidak ada perubahan sama sekali, kembalikan gambar asli
        if brightness == 0 and contrast == 1.0 and saturation == 1.0:
            return cv_image.copy()

        # --- Langkah 1: Kecerahan & Kontras di ruang BGR ---
        # Bekerja dengan tipe data float untuk menghindari kehilangan data (clipping)
        img_float = cv_image.astype(np.float32)

        # Terapkan kontras terlebih dahulu, baru kecerahan
        # Rumus: output = contrast * input + brightness
        img_float = img_float * contrast + brightness

        # Batasi (clip) nilainya agar tetap dalam rentang warna yang valid (0-255)
        img_float = np.clip(img_float, 0, 255)

        # Konversi kembali ke uint8 untuk langkah selanjutnya
        img_after_br_co = img_float.astype(np.uint8)

        # --- Langkah 2: Saturasi di ruang HSV ---
        # Jika saturasi tidak berubah, kita bisa berhenti di sini
        if saturation == 1.0:
            return img_after_br_co

        # Konversi gambar yang sudah disesuaikan kecerahan/kontrasnya ke HSV
        ...
        # Terapkan saturasi dengan mengalikan channel S
        s = s * saturation
        s = np.clip(s, 0, 255)  # Pastikan nilai saturasi tidak melebihi batas

        # Gabungkan kembali channel-channel tersebut
        final_hsv = cv2.merge((h, s, v))

        # Konversi kembali ke uint8 dan kemudian konversi kembali ke BGR
    ...
    def update_image_edits(self):
        ...
        # Panggil show_image_view tanpa memuat ulang gambar dari disk.
        # Ini akan menerapkan kalkulasi baru dari `apply_image_edits`
        # ke `self.original_cv_image` yang masih asli.
    ...
    def reset_image_edits(self):
        ...
        # Blokir sinyal agar tidak memicu update berkali-kali
        self.brightness_slider.blockSignals(True)
        self.contrast_slider.blockSignals(True)
        self.saturation_slider.blockSignals(True)
        ...
        # Hapus data editan dari file metadata jika ada
        ...
        # Panggil update secara manual sekali setelah reset
    ...
    def update_tag_display(self):
        ...
        if not self.current_image_path: 
            self.tag_display_layout.addStretch() # Tambahkan stretch jika kosong
            return
        ...
        for tag in tags:
            tag_widget = QFrame()
            tag_widget.setStyleSheet("QFrame { background-color: #4C566A; border-radius: 4px; }")
            ...
            remove_btn.setStyleSheet("background-color: #BF616A; border-radius: 10px; font-weight: bold; color: #ECEFF4;")
