import sys
import os
import cv2 # <-- [PENAMBAHAN] Library OpenCV ditambahkan untuk memuat gambar
import platform
import subprocess
import hashlib
import shutil
...
APP_VERSION = "1.0.1" # Versi diperbarui
...
class MacanGallery(QMainWindow):
    def __init__(self):
        ...
        # --- [PENAMBAHAN] Variabel untuk menyimpan pixmap viewer asli ---
    ...
    def init_ui(self):
        ...
        self.viewer_label = QLabel() # Hapus teks default
    ...
    def reflow_ui(self):
        """Redraws the UI based on the current view (folders or images)."""
    ...
    # --- [PERUBAHAN] Logika menampilkan gambar diubah total ---
    def show_image_view(self, path):
        """
        Loads an image using OpenCV for robustness and displays it in the viewer.
        This is adapted from macan_viewer1.py.
        """
        try:
            ...
            # 2. Convert from OpenCV's BGR/BGRA format to Qt's RGB/RGBA
            ...
            # 3. Create a QPixmap from the QImage
            pixmap = QPixmap.fromImage(qt_image)
            
            # 4. Store the full-resolution pixmap to prevent quality loss on resize
            self.current_viewer_pixmap = pixmap
            
            # 5. Display the scaled pixmap and switch views
            self.viewer_label.setPixmap(pixmap.scaled(
                self.viewer_scroll_area.size() - QSize(20,20), 
    ...
    def show_gallery_view(self):
        ...
        # --- [PENAMBAHAN] Hapus referensi pixmap untuk membebaskan memori ---
        self.current_viewer_pixmap = None
    ...
    def show_file_info(self, file_path):
        try:
            ...
            # Menggunakan imread hanya untuk mendapatkan dimensi
    ...
    # --- [PERUBAHAN] Logika resize diubah agar menskalakan ulang gambar viewer ---
    def resizeEvent(self, event):
        super().resizeEvent(event)
        # Timer untuk menunda penggambaran ulang grid thumbnail agar tidak berat
        ...
        # Jika view gambar sedang aktif, skalakan ulang gambar dari pixmap asli
        if self.main_stack.currentIndex() == 1 and self.current_viewer_pixmap:
             self.viewer_label.setPixmap(self.current_viewer_pixmap.scaled(
                 self.viewer_scroll_area.size() - QSize(20,20), 
                 Qt.AspectRatioMode.KeepAspectRatio, 
                 Qt.TransformationMode.SmoothTransformation
             ))
