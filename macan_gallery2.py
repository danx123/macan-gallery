...
class ThumbnailWidget(QToolButton):
    ...
    """
    # [FIX 1] Add a custom signal for double-clicking
    ...
    # [FIX 2] Implement the event handler to catch the double-click event
    def mouseDoubleClickEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            self.doubleClicked.emit() # Emit our custom signal
...
class MacanGallery(QMainWindow):
    ...
    def add_thumbnail_to_grid(self, path, pixmap):
        ...
        # This line is now correct because ThumbnailWidget has a 'doubleClicked' signal
        thumb_widget.doubleClicked.connect(lambda: self.show_image_view(path))
        
        # Calculate position in grid
        count = len(self.thumbnail_widgets)
    ...
    def reflow_grid(self):
        ...
        widgets = list(self.thumbnail_widgets.values())
    ...
    def resizeEvent(self, event):
        ...
        # Use a timer to avoid reflowing too frequently during a resize drag
        if not hasattr(self, 'resize_timer'):
            self.resize_timer = QTimer()
            self.resize_timer.setSingleShot(True)
            self.resize_timer.timeout.connect(self.reflow_grid)
