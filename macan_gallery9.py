...
APP_VERSION = "1.3.1" # [PERUBAHAN] Versi diperbarui dengan fitur pindai subfolder
...
class MacanGallery(QMainWindow):
    ...
    def start_scanning_folders(self):
        ...
        all_image_paths = []
        # [PERUBAHAN] Iterasi melalui setiap folder dasar yang ditambahkan pengguna
        for base_folder in folders:
            self.status_label.setText(f"Scanning {base_folder}...")
            QApplication.processEvents()
            try:
                # [PERUBAHAN] Gunakan os.walk untuk menelusuri folder dan semua subfoldernya
                for dirpath, _, filenames in os.walk(base_folder):
                    images_in_current_folder = []
                    for filename in filenames:
                        # Cek apakah file memiliki ekstensi gambar yang didukung
                        _, ext = os.path.splitext(filename)
                        if ext.lower() in SUPPORTED_IMAGE_EXTENSIONS:
                            full_path = os.path.join(dirpath, filename)
                            images_in_current_folder.append(full_path)

                    # Jika ada gambar di folder (subfolder) ini, tambahkan sebagai grup baru
                    if images_in_current_folder:
                        # Kunci kamus sekarang adalah path subfolder, bukan folder dasar
