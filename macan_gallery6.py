...
APP_VERSION = "1.1.0" # Versi diperbarui dengan fitur baru
...
class ThumbnailWidget(QFrame):
    ...
    # [PENAMBAHAN] Fungsi Drag and Drop
...
class FolderThumbnailWidget(QFrame):
    ...
    # [PENAMBAHAN] Fungsi Drag and Drop untuk folder
    def mousePressEvent(self, event: QMouseEvent):
        if event.button() == Qt.MouseButton.LeftButton:
            self.drag_start_position = event.position().toPoint()
        super().mousePressEvent(event)
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        # Ambil pixmap dari widget anak untuk preview drag
...
class MacanGallery(QMainWindow):
    ...
    def init_ui(self):
        ...
        # [PENAMBAHAN] Timer untuk debouncing pencarian
        self.search_timer = QTimer()
        self.search_timer.setSingleShot(True)
        self.search_timer.timeout.connect(self.reflow_ui)

    def apply_stylesheet(self):
        # [PERUBAHAN] Tema digelapkan dan disesuaikan
        self.setStyleSheet("""
            ...
            QStatusBar { 
                background-color: #3B4252; 
                color: #FFFFFF; /* [PERUBAHAN] Font putih eksplisit */
    ...
    def create_tool_bar(self):
        ...
        # [PENAMBAHAN] Bilah pencarian
        self.search_bar = QLineEdit()
        self.search_bar.setPlaceholderText("Search items...")
        self.search_bar.setMaximumWidth(300)
        self.search_bar.textChanged.connect(lambda: self.search_timer.start(300)) # Debounce
    ...
    def reflow_ui(self):
        """Redraws the UI based on the current view and search query."""
        ...
        # [PENAMBAHAN] Logika filter pencarian
        ...
        if self.current_view == 'folders':
            ...
            if search_term:
                sorted_folders = [f for f in sorted_folders if search_term in os.path.basename(f).lower()]
    ...
    def show_folder_contents(self, folder_path):
        ...
        self.search_bar.clear() # Clear search when entering a folder
        self.reflow_ui()
    ...
    def show_folders_view(self):
        ...
        self.current_view = 'folders'
        self.selected_folder = None
        self.search_bar.clear() # Clear search when going back to folders
        self.reflow_ui()
