...
class MacanGallery(QMainWindow):
    # GANTI KESELURUHAN FUNGSI __init__ DI KELAS MacanGallery
    ...
    def __init__(self):
        ...
        # [FITUR BARU] Dictionary untuk menyimpan nilai slider edit sementara
    ...
    # GANTI KESELURUHAN FUNGSI reflow_ui
    ...
    def reflow_ui(self):
        ...
        # [PERUBAHAN] Logika pencarian sekarang memeriksa nama file DAN tag
            if search_term:
                filtered_by_search = []
    ...
    # GANTI KESELURUHAN FUNGSI show_image_view
    ...
    def show_image_view(self, path, reload_from_disk=True):
        ...
        try:
            ...
            # Update slider tanpa memicu sinyal valueChanged
            ...
            if reload_from_disk:
                ...
                # Simpan versi original untuk editing
                if len(cv_image.shape) == 2: self.original_cv_image = cv2.cvtColor(cv_image, cv2.COLOR_GRAY2BGR)
                elif cv_image.shape[2] == 4: self.original_cv_image = cv2.cvtColor(cv_image, cv2.COLOR_BGRA2BGR)
                else: self.original_cv_image = cv_image
        
            # Terapkan editan
            edited_image = self.apply_image_edits(self.original_cv_image, self.current_edits)
        
            h_orig, w_orig, *_ = edited_image.shape
            qt_image = QImage(cv2.cvtColor(edited_image, cv2.COLOR_BGR2RGB).data, w_orig, h_orig, 3 * w_orig, QImage.Format.Format_RGB888)

            if qt_image.isNull(): raise Exception("Failed to convert the OpenCV image to a QImage.")
            pixmap = QPixmap.fromImage(qt_image)
            self.current_viewer_pixmap = pixmap
            # Simpan original pixmap untuk rotasi/flip, BUKAN untuk editan slider
            if reload_from_disk:
    ...
    def resizeEvent(self, event):
        ...
        if self.main_stack.currentIndex() == 1:
             ...
             # TAMBAHKAN FUNGSI BARU INI DI DALAM KELAS MacanGallery
    ...
    def _create_info_dock_widget(self):
        ...
        self.contrast_slider.setRange(0, 200) # Direpresentasikan sebagai 0.0 - 2.0
        ...
        self.saturation_slider.setRange(0, 200) # Direpresentasikan sebagai 0.0 - 2.0
        ...
        self.save_edits_button = QPushButton("Save Changes")
        ...
        self.tag_display_layout = QHBoxLayout() # Layout untuk menampilkan tag
        self.tag_display_layout.setSpacing(5)
        dock_layout.addLayout(self.tag_display_layout)
        ...
        self.metadata_viewer = QTextEdit()
        self.metadata_viewer.setReadOnly(True)
        self.metadata_viewer.setMinimumHeight(150)
    ...
    # TAMBAHKAN SEMUA FUNGSI BARU INI DI DALAM KELAS MacanGallery
    ...
    def apply_image_edits(self, cv_image, edits):
        ...
        # alpha untuk contrast, beta untuk brightness
        ...
        if saturation != 1.0:
            ...
            h, s, v = cv2.split(hsv)
            s = cv2.add(s, (saturation - 1.0) * 128) # Perubahan saturasi
            s = np.clip(s, 0, 255) # Pastikan nilai tidak keluar dari range
    ...
    # Hapus data edit dari metadata dan simpan
        meta = read_metadata(self.current_image_path)
        if 'edits' in meta:
            del meta['edits']
            write_metadata(self.current_image_path, meta)
            self.status_label.setText("Edits have been reset.")
    ...
    def update_tag_display(self):
        ...
        for tag in tags:
            ...
            remove_btn.setStyleSheet("background-color: #BF616A; border-radius: 10px; font-weight: bold;")
